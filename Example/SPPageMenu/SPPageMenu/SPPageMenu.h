...
typedef NS_ENUM(NSInteger, SPItemControlState) {
    SPItemControlStateNormal,       // 选中时的正常状态
    SPItemControlStateSelected,     // 选中时的选中状态
    SPItemControlStateOther         //不被选中时正常状态
};
typedef NS_ENUM(NSInteger, SPItemIconStatesNumber) {
    SPItemIconStatesNumberOne,       // 只有一次选中状态，默认只有一种图片状态
    SPItemIconStatesNumberTwo,       // 有两次选中状态，图片有两种
    SPItemIconStatesNumberThree      //有三次点击状态，图片有三种
};
...
@property (nonatomic, assign) CGFloat spaceLineMargin;
...
//图片有几种状态，默认一种
@property (nonatomic, assign) SPItemIconStatesNumber itemIconStatesNumber;//
//图片在有三中状态时，是否在第三中状态隐藏跟踪线（other,nor）
@property (nonatomic, assign) BOOL hiddenBoomLineAtThirdState;//默认不隐藏
...
@property (nonatomic, assign)  CGFloat trackerHeight; // 跟踪器的高度
...
@property (nonatomic, copy) UIImage *thirdImage;
