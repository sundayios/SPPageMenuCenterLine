...
@property (nonatomic, strong) UIImage *firstStateImage;
@property (nonatomic, strong) UIImage *thirdStateImage;
@end
...
//    _firstStateImage = [UIImage new];
//    _thirdStateImage = [UIImage new];
}
...
@property (nonatomic, assign) SPItemControlState currentItemControlState;
...
- (void)setItem:(SPPageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        [button setTitleColor:_selectedItemTitleColor forState:UIControlStateSelected];
        if (item.selTitle) {
            [button setTitle:item.selTitle forState:UIControlStateSelected];
        }else{
            [button setTitle:item.title forState:UIControlStateSelected];
        }
        if (item.selImage) {
            [button setImage:item.selImage forState:UIControlStateSelected];
        }else{
            [button setImage:item.image forState:UIControlStateSelected];
        }
        button.firstStateImage = item.image;
        if (item.thirdImage) {
            button.thirdStateImage = item.thirdImage;
        }else{
            button.thirdStateImage = item.image;
        }
...
- (void)setContent:(id)content forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        if ([content isKindOfClass:[NSString class]]) {
            ...
            [button setTitle:content forState:UIControlStateSelected];
            [button setTitleColor:_unSelectedItemTitleColor forState:UIControlStateNormal];
            [button setTitleColor:_selectedItemTitleColor forState:UIControlStateSelected];
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        switch (_itemIconStatesNumber) {
            case SPItemIconStatesNumberOne:
            {
            ...
            {
                [button setTitle:object forState:UIControlStateNormal];
                [button setTitle:object forState:UIControlStateSelected];
            }
                break;
        }
        ...
        switch (_itemIconStatesNumber) {
            case SPItemIconStatesNumberThree:
            {
                [button setImage:object forState:UIControlStateNormal];
                [button setImage:object forState:UIControlStateSelected];
                button.firstStateImage = object;
                button.thirdStateImage = object;
            }
                break;
            default:
            {
        ...
        [button setTitle:item.title forState:UIControlStateNormal];
        [button setImage:item.thirdImage forState:UIControlStateNormal];
        [button setTitle:item.selTitle forState:UIControlStateSelected];
        [button setImage:item.selImage forState:UIControlStateSelected];
        button.imagePosition = item.imagePosition;
        button.imageTitleSpace = item.imageTitleSpace;
    ...
    if (self.insert && animated) { // 是插入的新按钮,且需要动画
        ...
        if (index > 0 && index < self.numberOfItems && self.spaceLineColor) {
            UIView *spaceLineView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(lastButton.frame)+_itemPadding*0.25, _spaceLineMargin, 1, self.bounds.size.height - _spaceLineMargin * 2)];
            spaceLineView.backgroundColor = _spaceLineColor;
            [self.itemScrollView addSubview:spaceLineView];
        }
    }
}
...
- (void)initialize {
    ...
    _spaceLineMargin = 5.0;
    _currentItemControlState = SPItemControlStateOther;
    _itemIconStatesNumber = SPItemIconStatesNumberOne;
    _hiddenBoomLineAtThirdState = false;
    [self setupSubViews];
}
    ...
    if (self.selectedButton != sender) {
        ...
        switch (_itemIconStatesNumber) {
            case SPItemIconStatesNumberOne:
            {
                ...
                break;
            case SPItemIconStatesNumberTwo:
            {
                [sender setTitleColor:_selectedItemTitleColor forState:UIControlStateNormal];
                sender.titleLabel.font = _selectedItemTitleFont;
            }
            ...
            case SPItemIconStatesNumberThree:
            {
                if (self.selectedButton.thirdStateImage) {
                    [self.selectedButton setImage:self.selectedButton.thirdStateImage forState:UIControlStateNormal];
                }
                
                self.dividingLine.hidden = false;
                [sender setTitleColor:_unSelectedItemTitleColor forState:UIControlStateNormal];
                sender.titleLabel.font = _unSelectedItemTitleFont;
            }
                break;
            default:
                break;
        ...
        switch (_itemIconStatesNumber) {
            ...
            case SPItemIconStatesNumberTwo:
            {
                switch (_currentItemControlState) {
                    case SPItemControlStateNormal:
                    {
                        self.selectedButton.selected = true;
                        _currentItemControlState = SPItemControlStateSelected;
                    }
                        break;
                    case SPItemControlStateSelected:
                    {
                        self.selectedButton.selected = false;
                        _currentItemControlState = SPItemControlStateOther;
                    }
                ...
                [self.selectedButton setTitleColor:_selectedItemTitleColor forState:UIControlStateNormal];
                self.selectedButton.titleLabel.font = _selectedItemTitleFont;
            }
                break;
            case SPItemIconStatesNumberThree:
            {
                ///防止颜色设置滞后引起的问题
                [self.selectedButton setTitleColor:_unSelectedItemTitleColor forState:UIControlStateNormal];
                [self.selectedButton setTitleColor:_selectedItemTitleColor forState:UIControlStateSelected];
                switch (_currentItemControlState) {
                    case SPItemControlStateNormal:
                    {
                        self.selectedButton.selected = true;
                    ...
                    case SPItemControlStateSelected:
                    {
                        ...
                        self.dividingLine.hidden = _hiddenBoomLineAtThirdState;
                    }
                        break;
                    case SPItemControlStateOther:
                    {
                        self.selectedButton.selected = false;
                        [self.selectedButton setTitleColor:_selectedItemTitleColor forState:UIControlStateNormal];
                        self.selectedButton.titleLabel.font = _selectedItemTitleFont;
                        if (self.selectedButton.firstStateImage) {
                            [self.selectedButton setImage:self.selectedButton.firstStateImage forState:UIControlStateNormal];
                        }
                        _currentItemControlState = SPItemControlStateNormal;
                        self.dividingLine.hidden = false;
                    }
                        break;
                    default:
...
- (void)setTrackerHeight:(CGFloat)trackerHeight {
    _trackerHeight = trackerHeight;
    CGRect trackerRect = self.tracker.frame;
    trackerRect.size.height = trackerHeight;
    self.tracker.frame = trackerRect;
    CGPoint trackerCenter = self.tracker.center;
    trackerCenter.x = _selectedButton.center.x;
    self.tracker.center = trackerCenter;
...
- (void)setSpaceLineMargin:(CGFloat)spaceLineMargin{
    _spaceLineMargin = spaceLineMargin;
    [self setNeedsLayout];
    [self layoutIfNeeded];
}
...
- (void)setSelectedItemTitleColor:(UIColor *)selectedItemTitleColor {
    ...
    if (self.itemIconStatesNumber == SPItemIconStatesNumberOne) {
        [self.selectedButton setTitleColor:selectedItemTitleColor forState:UIControlStateNormal];
    }else{
        [self.selectedButton setTitleColor:selectedItemTitleColor forState:UIControlStateSelected];
    }
...
- (void)layoutSubviews {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(SPPageMenuButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == SPPageMenuPermutationWayScrollAdaptContent) {
            ...
            if (idx > 0 && idx < self.numberOfItems && self.spaceLineColor) {
                UIView *spaceLineView = [[UIView alloc] initWithFrame:CGRectMake(lastButtonMaxX+customSpacing*0.25, _spaceLineMargin, 1, self.bounds.size.height - _spaceLineMargin * 2)];
                spaceLineView.backgroundColor = _spaceLineColor;
                [self.itemScrollView addSubview:spaceLineView];
            }
            ...
            if (idx > 0 && idx < self.numberOfItems && self.spaceLineColor) {
                UIView *spaceLineView = [[UIView alloc] initWithFrame:CGRectMake(lastButtonMaxX, _spaceLineMargin, 1, self.bounds.size.height - _spaceLineMargin * 2)];
                spaceLineView.backgroundColor = _spaceLineColor;
                [self.itemScrollView addSubview:spaceLineView];
            }
        } else {
...
+ (instancetype)itemWithNorTitle:(NSString *)norTitle selTitle:(NSString *)selTitle norImage:(UIImage *)norImage selImage:(UIImage *)selImage thirdImage:(UIImage *)thirdImage {
    SPPageMenuButtonItem *item = [[SPPageMenuButtonItem alloc] initWithNorTitle:norTitle selTitle:selTitle norImage:norImage selImage:selImage thirdImage:thirdImage imagePosition:SPItemImagePositionDefault];
    return item;
}
// 快速创建同时含有标题和图片的item，imagePositiona参数为图片位置
+ (instancetype)itemWithNorTitle:(NSString *)norTitle selTitle:(NSString *)selTitle norImage:(UIImage *)norImage selImage:(UIImage *)selImage thirdImage:(UIImage *)thirdImage imagePosition:(SPItemImagePosition)imagePosition {
    SPPageMenuButtonItem *item = [[SPPageMenuButtonItem alloc] initWithNorTitle:norTitle selTitle:selTitle norImage:norImage selImage:selImage thirdImage:thirdImage imagePosition:imagePosition];
    return item;
}
...
- (instancetype)initWithNorTitle:(NSString *)norTitle selTitle:(NSString *)selTitle norImage:(UIImage *)norImage selImage:(UIImage *)selImage thirdImage:(UIImage *)thirdImage imagePosition:(SPItemImagePosition)imagePosition {
    if (self = [super init]) {
        self.title = norTitle;
        self.image = norImage;
